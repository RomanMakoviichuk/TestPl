...
document.addEventListener("DOMContentLoaded", function () {
  ...
  let mainSliderImages = document.querySelectorAll(
    ".main-slider-top .swiper-slide img"
  );
  let zoomBtns = document.querySelectorAll(
    ".main-slider-top .swiper-slide .zoom-btn"
  );
  ...
  document.addEventListener("click", function (event) {
    if (
      !event.target.matches(".main-slider-top .swiper-slide img") &&
      !event.target.matches(".main-slider-top .swiper-slide .zoom-btn")
    ) {
...
function handleRadioButtonChange(radioButton) {
  ...
  totalDiscount.textContent = "Oszczędź " + formatPrice(oldPrice - actualPrice);
  "OSZCZĘDZASZ " + formatPrice((oldPrice - actualPrice) * selectedPackage);

  // update link for selected package
  if (Number(selectedPackage) === 3) {
    addToCartLink.href = "https://swisspower.pl/cart/50690771648861:3";
    totalOldPrice.classList.remove("hide");
    totalDiscount.classList.remove("hide");
  } else if (Number(selectedPackage) === 1) {
    addToCartLink.href = "https://swisspower.pl/cart/50690771648861:1";
    totalOldPrice.classList.add("hide");
    totalDiscount.classList.add("hide");
  } else {
    addToCartLink.href =
      "https://swisspower.pl/cart/50690771648861:" + selectedPackage;
    totalOldPrice.classList.remove("hide");
    totalDiscount.classList.remove("hide");
  }
  ...
  radioButtons.forEach((otherRadioButton) => {
    otherRadioButton.parentElement.classList.toggle(
      "selected",
...
for (let anchor of anchors) {
  anchor.addEventListener("click", function (e) {
    ...
    if (targetElement) {
      ...
      const offset =
        window.pageYOffset + elementOffset - window.innerHeight / 3;
